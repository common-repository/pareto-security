...
class pareto_functions extends pareto_setup {
    ...
    public function _POST_SHIELD() {
        ...
        if ( !isset( $_SERVER[ 'REQUEST_METHOD' ] ) || false === $this->cmpstr( 'POST', $_SERVER[ 'REQUEST_METHOD' ], true ) )
            return; // of no interest to us
        $_get_post   = $_POST;
        ...
        if ( false !== $this->is_wp() ) {
                ...
                if ( false !== strpos( $HTTP_RAW, '_IWP_JSON_PREFIX_' ) ) {
                    ...
                    if ( false !== strpos( $IWP_HTTP_RAW, 'iwp_action' ) &&
                       ( false !== strpos( $IWP_HTTP_RAW, 'add_site' ) || false !== strpos( $IWP_HTTP_RAW, 'readd_site' ) ) ) {
    ...
    function htaccess_unbanip( $allips = true, $ip = '' ) {
        ...
        foreach( $mybans as $bans ) {
            if ( false !== strpos ( $bans, trim( $limitstart ) ) ||
                 false !== strpos ( $bans, trim( $limitstart2 ) ) ) {
                 $is_ps_ban = true;
            }
        }
